...
" <<<"$jwt_json")
    ...
    [[ -n "$rtmr_0" ]] && log "    RTMR 0 (TDVF config):       ${rtmr_0:0:32}..."
    [[ -n "$rtmr_1" ]] && log "    RTMR 1 (Bootloader/Kernel): ${rtmr_1:0:32}..."
    [[ -n "$rtmr_2" ]] && log "    RTMR 2 (Cmdline/Initrd):    ${rtmr_2:0:32}..."
    [[ -n "$rtmr_3" ]] && log "    RTMR 3 (Runtime/Guest OS):  ${rtmr_3:0:32}..."
